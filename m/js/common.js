$(function(){
			...
			case 'community': 
				$(".contents .snb li a").on("click",function(){ 
					...
					if(nb == 0){
						...
						$(".frame h3 span").text("아이놀터에 무엇이든 물어보세요.");
	...
	$.getJSON('http://digitalnow.co.kr/reserve/pensionInfo/'+ account +'/4',	//User Info
		function(data){
			var USER_TEL1 = data.result.USER_TEL1;				//전화번호 1
			var USER_TEL2 = data.result.USER_TEL2;				//전화번호 2
			var NEW_USER_ADDR = data.result.NEW_USER_ADDR;		//도로명주소
			var USER_ADDR = data.result.USER_ADDR;				//지번주소
			var BUSI_NM = data.result.BUSI_NM;					//상호명
			var USER_EMAIL = data.result.USER_EMAIL;			//이메일
			var USER_ACCO = data.result.USER_ACCO;				//계좌번호
			var COMM_SALE_NO = data.result.COMM_SALE_NO;		//통신판매업번호
			...
			$("footer").append(
				...
				'<div id="ft-cont">' +
					'<div class="InBox">' +
						'<ul class="address">' +
							'<li><a href="tel:'+ USER_TEL1 +'">예약문의<span>'+ USER_TEL1 +'</span> <img src="../images/tell.png" width="38" height="auto" /></a></li>' +
					...
					'<div class="copy">' +
						'<p>' +
							'업체명 :' + BUSI_NM +   ' | 대표자 : <span class="BUSI_PRE_NM"></span><br />' +
							'사업자번호: <span class="BUSI_NO"></span>' + '<br />' +
							'통신판매업 번호 : '+ COMM_SALE_NO +'<br />' +
							'계좌번호 : ' + USER_ACCO + 
						'</p>' +
			...
			function(data){
				var BUSI_PRE_NM = new Array(); 						//사업자번호
				var BUSI_NO = new Array();							//대표자
				$.each(data.result,function(key,val){
